                ...
                <div key={order.id} className="p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <select
                      ...
                      className={`text-xs font-medium px-2.5 py-1.5 rounded-full border-0 cursor-pointer flex-shrink-0 ${STATUS_BADGE[order.status] || 'bg-gray-100 text-gray-700'}`}
                    >
                      {STATUSES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                    </select>
                  </div>
                  <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
                    <span>{order.items?.length || 0} поз. · {new Date(order.createdAt).toLocaleDateString('ru-RU')}</span>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((order: any) => (
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500">{order.items?.length || 0} поз.</td>
                      <td className="px-6 py-4 font-semibold text-sm text-gray-900 whitespace-nowrap">{Number(order.totalAmount || 0).toLocaleString('ru-RU')} ₽</td>
                      <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{new Date(order.createdAt).toLocaleDateString('ru-RU')}</td>

                ...
                <div key={order.id} className="p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      ...
                      <div className="font-medium text-sm text-gray-800 mt-0.5">{order.customerName || '—'}</div>
                      {order.customerPhone && <div className="text-xs text-gray-400">{order.customerPhone}</div>}
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium flex-shrink-0 ${STATUS_BADGE[order.status] || 'bg-gray-100 text-gray-700'}`}>
                      {STATUS_LABEL[order.status] || order.status}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    <span>
                      {fmt(order.startDate) ? `${fmt(order.startDate)} — ${fmt(order.endDate) || '?'}` : 'Без дат'}
                    </span>
                    <span className="font-semibold text-gray-900 text-sm">{Number(order.totalAmount || 0).toLocaleString('ru-RU')} ₽</span>
                  </div>
                  <div className="flex gap-2 mt-3">
                    {order.endDate && CAN_EXTEND.includes(order.status) && (
                      <button onClick={() => openExtend(order)}
                        className="flex-1 flex items-center justify-center gap-1 text-xs text-amber-600 border border-amber-200 bg-amber-50 hover:bg-amber-100 px-3 py-2 rounded-lg transition-colors">
                        <CalendarPlus className="w-3.5 h-3.5" /> Продлить
                      </button>
                    )}
                    <Link href={`/orders/${order.id}`}
                      className="flex-1 flex items-center justify-center gap-1 text-sm text-blue-600 border border-blue-200 bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-lg transition-colors">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((order: any) => (
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4 font-mono font-medium text-sm text-gray-900 whitespace-nowrap">{order.orderNumber}</td>
                      <td className="px-6 py-4 min-w-0">
                        <div className="font-medium text-sm text-gray-900 truncate max-w-[160px]">{order.customerName || '—'}</div>
                      ...
                      <td className="px-6 py-4 text-xs text-gray-500 whitespace-nowrap">
                        {fmt(order.startDate) ? <span>{fmt(order.startDate)} — {fmt(order.endDate) || '?'}</span> : <span className="text-gray-300">без дат</span>}
                      </td>

  ...
  async feedYml(@Res() res: Response) {
    ...
    const offers = products.map((p) => {
      ...
      return [
        ...
        p.brand ? `    <vendor>${esc(p.brand)}</vendor>` : `    <vendor>КаякРент</vendor>`,
    ...
    const xml = [
      ...
      `<shop>`,
      `  <name>${esc(this.config.get("SHOP_NAME") || "КаякРент")}</name>`,
      `  <company>${esc(this.config.get("SHOP_NAME") || "КаякРент")}</company>`,
      `  <url>${siteUrl}</url>`,

...
if [ "${BOOTSTRAP_SUPERADMIN_CREATE:-false}" = "true" ] && [ -n "${BOOTSTRAP_SUPERADMIN_EMAIL:-}" ]; then
  ...
  echo -e "${BOLD}${YELLOW}│  ДАННЫЕ ДЛЯ ВХОДА В АДМИНКУ — СОХРАНИТЕ!        │${RESET}"
  echo -e "${BOLD}${YELLOW}└──────────────────────────────────────────────────┘${RESET}"
  echo -e "  ${CYAN}URL:${RESET}       ${APP_URL:-http://YOUR_SERVER_IP}/crm"
  echo -e "  ${CYAN}Email:${RESET}     ${BOOTSTRAP_SUPERADMIN_EMAIL}"
  echo -e "  ${CYAN}Пароль:${RESET}    ${BOOTSTRAP_SUPERADMIN_PASSWORD:-CHANGE_ME_ADMIN_PASSWORD}"
  echo ""
  echo -e "  ${RED}⚠  После первого входа ОБЯЗАТЕЛЬНО смените пароль!${RESET}"
  echo -e "  ${RED}   CRM → Профиль → Сменить пароль${RESET}"
  echo ""
fi
